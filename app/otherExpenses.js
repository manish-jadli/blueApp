...
export default class OtherExpenses extends React.Component {
    ...
    submitDeposit=()=>{
        if(this.state.username && this.state.date && this.state.billDetail && this.state.accountType && this.state.bank && this.state.totalAmount){
          ...
          fetch(`https://angular2-9b951.firebaseio.com/otherExpenses.json`,{
            method: 'POST',
            headers:{
    ...
    //         if(error)
    //         ToastAndroid.show('Error whent init SoundPlayer : (((', ToastAndroid.SHORT);
    //     });
    // }
    ...
    //             ToastAndroid.show('Error whent init SoundPlayer : (((', ToastAndroid.SHORT);
    //         });
    //         Alert.alert('Sound','You have played to sound.');
    //     }
    // }
...
<Picker
  ...
  <Picker.Item label="Previous Tax Payable" value="Previous Tax Payable" />
